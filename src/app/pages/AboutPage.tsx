    ...
    >
      <Section>
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="relative">
            ...
            <div className="absolute -bottom-5 -right-5 px-6 py-5" style={{ backgroundColor: "#A0780E" }}>
              <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: "2rem", color: "#fff", lineHeight: 1 }}>300+</div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[{ num: "7,000+", label: "Clients Worldwide" }, { num: "20+", label: "Countries" }, { num: "300+", label: "Installations" }, { num: "10+", label: "Industries" }].map(s => (
              <div key={s.label} className="p-6 text-center" style={{ backgroundColor: "#ffffff", border: "1px solid rgba(23,139,76,0.15)" }}>
                <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: "2.2rem", color: "#178B4C", lineHeight: 1 }}>{s.num}</div>
      ...
      <Section bg="#F5F4EF">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {values.map((v, i) => <FeatureCard key={i} title={v.title} description={v.desc} icon={v.icon} />)}
        </div>
      </Section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {team.map((member, i) => (
            <motion.div key={member.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.07 * i }}
              className="text-center p-6" style={{ border: "1px solid rgba(23,139,76,0.15)" }}>
              <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: "#053114" }}>
                <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: "1.2rem", color: "#A0780E" }}>{member.initials}</span>
              </div>
